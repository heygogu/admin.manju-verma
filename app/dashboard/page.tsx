    ...
    <div className="space-y-8 grid grid-cols-1">
      <div className="flex justify-between items-center">
        ...
        <Select  value={type} onValueChange={
          (value)=>{setType(value);fetchDashboardCards(value)}
        }>
      ...
      <Suspense fallback={<DashboardCardSkeleton />}>
            <div className="grid gap-4 md:grid-cols-3 ">
           { Array.isArray(dashboardCards) && dashboardCards.length ? dashboardCards?.map((stat, index) => (
                <motion.div
                key={stat?.title}
                initial={{ opacity: 0, y: 20 }}
                ...
                <Card className="shadow-lg hover:shadow-xl transition-shadow duration-300">
                  ...
                  {
                    stat?.title === "Total Clients" ? <Users className="h-6 w-6 text-blue-500" /> :
                    stat?.title === "Blog Posts" ? <FileText className="h-6 w-6 text-green-500" /> :
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    {type === "today" ? "Today" : type === "weekly" ? "This Week" : type === "monthly" ? "This Month" : "This Year"}
                  </div>
      ...
      <div className="space-y-4">
        ...
        <RecentClientsTable isLoading={isLoading} recentClients={recentClients} />
      </div>
    </div>
    </PageContainer>
  )
