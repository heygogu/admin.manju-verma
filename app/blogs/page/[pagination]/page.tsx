      ...
      cell: ({ row }:{row:any}) => (
        <div className="flex items-center gap-2 w-56 ml-4">
          ...
          <div className="flex flex-col">
            <span className="font-medium">{row.original?.title?.length>20?row.original?.title?.slice(0,20)+"...":row.original.title}</span>
            <span className="text-gray-500">{row.original.excerpt?.length>30?row.original?.excerpt?.slice(0,30)+"...":row.original.excerpt}</span>
          </div>
        </div>
      ),
    },
    ...
    },
{
      ...
      cell: ({ row }:{row:any}) => (
        <div className="flex flex-wrap gap-1">
          {row.original?.tags?.slice(0,5)?.map((tag:string, index:number) => (
            <Badge key={index} variant="default" className="bg-gray-100 capitalize text-gray-800">
              {tag}
            </Badge>
          ))}
          {
            row.original?.tags?.length>5&&<Badge variant="default" className="bg-gray-100 text-gray-800">+{row.original?.tags?.length-5}</Badge>
          }
        </div>
      ),
}
    ...
    
    {
      ...
      cell: ({ row }:{row:any}) => (
        <Badge
          variant={row.original.status === "published" ? "default" : "destructive"}
          className={`shadow-md ${
    ...
    },
    {
      ...
      cell:({row}:{row:any})=>(
        <span>{dayjs(row.original.publishDate).format("D MMM YYYY, h:mm A")}</span>
      )
    },
    {
      id: "actions",
